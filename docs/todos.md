下面是一个算法调度的组合式实现。 
用户写各种 Algorithm 类，Algorithm 类的接口不能变，它接受 DataContext，从中获取数据，或者 Create 数据给下游传递。
我设计了 Processor 的组合机制，以及构造器组合成各种 Processor，最后通过 Scheduler 运行 Processor。 Processor 之间可以串行、并行、竞争执行。
现在我需要增加一个 DataParallelProcessor，它的构造器的参数是一个数据类型 T 和一个其它的 Processor 构造器（可能是个复合 Processor 构造器）；DataParallelProcessor的实现中可以根据 DataContext 中的某个数据类型的实例数，把数据实例 ID 分发给多个 Processor 的不同实例并行执行，不同 Processor 实例ke'y
请实现 DataParallelProcessor，并写出测试用例。
